  ...
  <div class="max-w-980 mx-auto mb-4">
    <div class="">
      <form class="flex flex-col w-full max-w-sm mx-auto">
        <div class="mb-4">
          <input v-model="nickname" class="flex-1 appearance-none
          ...
          >
          <p class="text-red-500 text-sm">{{ nicknameError }}</p>
        </div>
        <div>
      ...
      text-white text-base font-semibold py-2 px-4 rounded-lg
        ...
        focus:outline-none focus:ring-2 focus:ring-green-500
        focus:ring-offset-2 focus:ring-offset-purple-200" type="button" @click="doSignUp"
        >
          Sign-up
        </button>
      </form>
...
export default defineComponent({
  ...
  setup () {
    ...
    const doSignUp = () => {
      if (!isValidNickname() || !isValidEmail() || !isValidPassword()) return
      const payload = { nickname: nickname.value, email: email.value, password: password.value }
      vuex.authStore.signUp(payload)
      router.push('/')
    }
